...
namespace Emsal.BLL
{
    public partial class BusinessLogic
    {
        ...
        public BaseOutput GetPersons(BaseInput baseInput, out List<tblPerson> Persons)
        {
            BaseOutput baseOutput;
            try
            {
                Persons = operationLogic.GetPersons();
                return baseOutput = new BaseOutput(true, BOResultTypes.Success.GetHashCode(), BOBaseOutputResponse.SuccessResponse, "");
            }
            catch (Exception ex)
            {
        ...
        public BaseOutput GetAnnouncements(BaseInput baseinput, out List<tblAnnouncement> itemList)
        {
            BaseOutput baseOutput;
            try
            {
                itemList = operationLogic.GetAnnouncements();
                return baseOutput = new BaseOutput(true, BOResultTypes.Success.GetHashCode(), BOBaseOutputResponse.SuccessResponse, "");
        ...
        public BaseOutput GetAnnouncementDetailsByProductId(BaseInput baseinput, Int64 productID, out List<AnnouncementDetail> itemList)
        {
            ...
            try
            {
                ...
                foreach (var obj in tblAnouncementlist)
                {
                    ...
                    item.productCatalogDocumentList = operationLogic.GetProductDocumentsByProductCatalogId((long)obj.product_id);
                    itemList.Add(item);
                }
